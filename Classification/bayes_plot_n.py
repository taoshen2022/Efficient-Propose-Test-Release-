...
def metric_display_name(metric: str) -> str:
    ...
    if metric == "balanced_error":
        return "balanced test error"
    raise ValueError(f"Unknown metric: {metric}")



def metric_file_tag(metric: str) -> str:
    if metric == "misclassification":
...
def evaluate_one_rep(
    ...
    out["Nonprivate"] = compute_test_metric(y_test, y_pred_np, metric=metric, K=K)

    etpr = fit_bayes_etpr(
        X=X_train, y=y_train, eps=eps, delta=delta, Rx=Rx, c0=c0, K=K, seed=3000 + rep_seed
    )
    out["ePTR"] = compute_test_metric(
        y_test,
    ...
    out["dpnbss"] = compute_test_metric(
        y_test,
...
def plot_summary(summary, x_col, x_label, title, y_label, out_png):
    label_map = {
    ...
    # softer background / cleaner look
    ax.set_facecolor("#f7f7f7")
    for spine in ["top", "right"]:
        ax.spines[spine].set_visible(False)

    for method in ["Nonprivate", "ePTR", "dpnb", "dpnbss"]:
        ...
        # shaded variability band: mean ± std
        ax.fill_between(
            x,
            y - s,
            y + s,
...
def main():
    parser = argparse.ArgumentParser(description="Plot Bayes test error versus n for privacy baselines.")
    parser.add_argument("--reps", type=int, default=500)
    parser.add_argument("--n-test", type=int, default=100000)
    parser.add_argument("--outdir", type=str, default="./Classification/results_n")
    ...
    parser.add_argument("--eps-split-counts", type=float, default=0.4)
    parser.add_argument(
        "--metric",
        type=str,
        default="balanced_error",
        choices=["misclassification", "balanced_error"],
        help="Test metric to plot.",
    )
    ...
    n_grid = [1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000]
    K = 3
    ...
    for n_train in n_grid:
        for rep in range(args.reps):
            errs = evaluate_one_rep(
                ...
                metric=args.metric,
                rep_seed=100000 * n_train + rep,
            )
            for method, err in errs.items():
                rows.append({"n": n_train, "rep": rep, "method": method, "error": err})

    raw = pd.DataFrame(rows)
    summary = summarize_results(raw, "n")

    tag = metric_file_tag(args.metric)
    y_label = (
        "Balanced test error"
        if args.metric == "balanced_error"
        else "Test misclassification rate"
    )
    title = rf"{y_label} vs. sample size ($\varepsilon={args.eps}$)"

    raw_csv = outdir / f"bayes_error_vs_n_{tag}_raw.csv"
    summary_csv = outdir / f"bayes_error_vs_n_{tag}_summary.csv"
    plot_png = outdir / f"bayes_error_vs_n_{tag}.png"
    ...
    plot_summary(
        summary=summary,
        x_col="n",
        x_label="Training sample size n",
        title=title,
        y_label=y_label,
        out_png=plot_png,
    )

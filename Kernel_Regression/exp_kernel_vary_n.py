...
def summarize_rows(rows: list[dict], x_key: str) -> list[dict]:
    ...
    out.sort(key=lambda d: (int(d[x_key]), METHOD_ORDER.index(d["method"])))
    return out
...
def make_plot(summary_rows: list[dict], out_path: Path, eps: float) -> None:
    ...
    for method in METHOD_ORDER:
        ...
        xs = np.array([int(r["n"]) for r in sub], dtype=float)
    ...
    ax.set_title(
        rf"Mean pointwise error vs. sample size ($\varepsilon={eps}$)",
        fontsize=18,
        fontweight="bold",
        pad=12,
    )
...
def main() -> None:
    ...
    parser.add_argument("--n-grid", type=str, default="500,1000,1500,2000,2500,3000,3500,4000,4500,5000")
    ...
    parser.add_argument("--outdir", type=str, default="./KernelRegression/results_vs_n")
    ...
    summary_rows = summarize_rows(raw_rows, "n")

    write_csv(
        outdir / "raw_vs_n.csv",
        raw_rows,
        ["n", "rep", "method", "theta", "se", "released", "truth"],
    )
    write_csv(
        outdir / "summary_vs_n.csv",
        summary_rows,
        ["n", "method", "mean_se", "sd_se", "mean_release"],
    )
    make_plot(summary_rows, outdir / "plot_vs_n.png", eps=args.eps)
